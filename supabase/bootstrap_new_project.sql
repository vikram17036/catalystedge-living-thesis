...
create policy "Users select own thesis_dependencies"
  on public.thesis_dependencies for select using (auth.uid() = user_id);
create policy "Users insert own thesis_dependencies"
  on public.thesis_dependencies for insert with check (auth.uid() = user_id);
create policy "Users delete own thesis_dependencies"
  on public.thesis_dependencies for delete using (auth.uid() = user_id);
